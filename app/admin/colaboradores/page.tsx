...
export default async function ColaboradoresPage() {
  ...
  return (
    <main className="min-h-screen w-full overflow-x-hidden bg-[#050816] px-4 py-8 text-white sm:px-6 md:px-10">
      <div className="mx-auto w-full max-w-6xl space-y-8">
        <header className="flex w-full flex-col gap-5 border-b border-white/10 pb-6 md:flex-row md:items-center md:justify-between">
          ...
          <div className="grid w-full grid-cols-1 gap-3 sm:grid-cols-2 md:w-auto">
            <Link
              href="/admin"
              className="inline-flex w-full items-center justify-center gap-2 rounded-2xl border border-white/10 bg-white px-4 py-3 text-sm font-black text-slate-950 shadow-lg transition hover:scale-[1.02] hover:bg-cyan-50"
            >
              <ArrowLeft size={17} />
              Voltar
            </Link>
            ...
            <Link
              ...
              className="inline-flex w-full items-center justify-center gap-2 whitespace-nowrap rounded-2xl bg-cyan-400 px-4 py-3 text-sm font-black text-slate-950 shadow-lg shadow-cyan-500/20 transition hover:scale-[1.02] hover:bg-cyan-300"
            >
              <Plus size={18} />
              Novo colaborador
            </Link>
        ...
        <section className="w-full rounded-3xl border border-white/10 bg-white/[0.04] p-4 shadow-2xl shadow-black/30 backdrop-blur sm:p-6">
                  ...
                  <div className="flex min-w-0 items-start gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="mt-1 flex min-w-0 items-start gap-2 text-xs font-medium leading-relaxed text-slate-400 sm:text-sm">
                        <Mail size={15} className="mt-[2px] shrink-0 text-cyan-300" />
                        <span className="min-w-0 break-all">{colaborador.email}</span>
                      </p>
                    </div>
                  </div>

                  <div className="mt-5 grid gap-3">
                    <div className="break-words rounded-2xl border border-white/10 bg-[#050816] p-3 text-sm text-slate-300">
                    ...
                    <div className="break-words rounded-2xl border border-white/10 bg-[#050816] p-3 text-sm text-slate-300">
                      <strong className="text-slate-100">Cadastrado em:</strong>{" "}
                      {formatDate(colaborador.createdAt)}
                    </div>

                    <BotaoEditarColaborador colaboradorId={colaborador.id} />
                    <BotaoExcluirColaborador colaboradorId={colaborador.id} />
                  </div>
                </div>
              ))}
            </div>
